...
export default function Navbar() {
  ...
  return (
    <div className="relative w-full ">
      ...
      <div className="hidden lg:w-full lg:flex items-center py-8 border-t-2 border-t-black justify-between px-6">
        ...
        {token ? (
          <div className="flex gap-4 text-md font-semibold items-center">
            ...
            <Link href={"/cart"}>
              <ShoppingCart className="cursor-pointer" size={35} />
            </Link>
          </div>
        ) : (
          <Link className="text-lg font-semibold" href={"/auth/login"}>
            Login
          </Link>
        )}
      </div>
      ...
      {/* Menu */}
      <div
        className={`fixed lg:hidden left-0 top-0 w-full h-screen flex-col z-50 bg-gray-600 bg-opacity-55
