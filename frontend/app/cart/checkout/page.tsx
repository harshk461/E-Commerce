...
export default function CheckOut() {
    ...
    const getAllAddress = async () => {
        const token = localStorage.getItem("token");
        setLoading(true);
        await axios.get("http://localhost:3002/auth/get-address", { headers: { Authorization: "Bearer " + token } })
            .then(res => {
    ...
    const applyPromo = async () => {
        await axios.get(url + "/order/check-promo/" + promoCode)
            .then(res => {
                setDiscount({
        ...
        <div className='w-full h-full flex flex-col'>
            ...
            <div className='max-w-full w-[1000px] m-auto p-4 flex flex-col gap-4'>
                <div className='w-full flex flex-col gap-3 p-3 border-2 border-black rounded-md'>
                    <h1 className='text-lg font-semibold text-gray-600'>Select Address</h1>
                    <div className='w-full flex gap-2 overflow-y-scroll'>
                        {addresses.map((item, i) => (
                            <div
                                onClick={() => setCurrAddress(i)}
                                className={`w-[200px] p-2 flex flex-col border-2 border-black rounded-lg gap-2 ${i === currAddress && 'border-yellow-400'}`}
                                key={i}>
                                <h1 className='text-lg font-semibold'>{item.name}</h1>
                                <h1 className='text-md text-gray-600'>{item.address},{item.city},{item.state},{item.country},{item.pincode}</h1>
